...
file_path = r'C:\Users\vitor\projects\exe-pro\app.html'

with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# Find the renderAgenda function
pattern = r'(\s+)async function renderAgenda\(\) \{'
match = re.search(pattern, content)
if not match:
    print("renderAgenda not found")
    exit(1)
...
print(f"Found renderAgenda from {start} to {end}")

new_func = '''    async function renderNotas() {
            ...
            <div class="notas-lista" id="notas-lista">
              ${filtered.length ? filtered.map((n, idx) => `
                <div class="nota-item ${(appState.notaSelecionada && appState.notaSelecionada.id === n.id) ? "active" : ""}" data-nota-index="${idx}">
                  ...
                  <div class="nota-item-meta">${formatDateHora(n.updatedAt || n.createdAt)}${(n.tags || []).length ? ' · ' + (n.tags || []).slice(0, 2).map(t => `<span class="nota-tag-chip">${t}</span>`).join("") : ""}</div>
                </div>
          ...
          <div class="notas-editor" id="notas-editor">
            ${appState.notaSelecionada ? (() => {
              ...
              return `
                ...
                <div class="notas-editor-pes">${n.audioTranscrito ? `<span class="nota-pill"><i data-lucide="mic"></i> Transcrita por voz</span>` : `<span class="nota-pill">${formatDateHora(n.updatedAt || n.createdAt)}</span>`}</div>
              `;
            })() : `
